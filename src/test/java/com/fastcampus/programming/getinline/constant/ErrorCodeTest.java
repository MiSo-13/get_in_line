package com.fastcampus.programming.getinline.constant;
...
class ErrorCodeTest {
    ...
    void giveExceptionWithMessage_whenGettingMessage_thenReturnMessage(ErrorCode sut, String expected) {
        // Given
    ...
    static Stream<Arguments> giveExceptionWithMessage_whenGettingMessage_thenReturnMessage() {
        return Stream.of(
    ...
    @ParameterizedTest(name = "[{index}] \"{0}\" ----> \"{1}\"")
    @MethodSource
    @DisplayName("예외 메시지를 받으면, 해당 에러 메시지로 출력")
    void giveMessage_whenGettingMessage_thenReturnMessage(String input, String expected) {
        // Given
    ...
    void givenErrorCode_whenToString_thenReturnsSimplifiedToString() {
        ...
        // Then
        assertThat(result)
                .isEqualTo("INTERNAL_ERROR {20000}");
    }
}
